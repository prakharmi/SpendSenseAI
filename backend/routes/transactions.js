...
async function extractTextFromImage(imageBuffer, mimeType) {
    ...
    // FINAL FIX: Use the stable 'gemini-pro-vision' model for image analysis.
...
router.post('/import-pdf', isLoggedIn, upload.single('pdf'), async (req, res) => {
    ...
    try {
        const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
        // FINAL FIX: Use the stable 'gemini-pro' model for this task.
...
// ... the rest of your transaction routes remain the same ...
